...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Si non trouvé, vérifier dans la table professionelle
    if (!$user) {
        $user = checkUser($connexion, 'professionelle', 'EmailEntreprise', $email, $password);
        $userType = 'professionnel';
    }

    // Si toujours non trouvé, vérifier dans la table particulier
    if (!$user) {
...
// Vérification de la connexion avant d'accéder à la page étudiant
